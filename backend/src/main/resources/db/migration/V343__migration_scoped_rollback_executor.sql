-- Rollback is the one deliberate hard-removal path for CRM rows created by a
-- migration. Keep DELETE away from the general runtime role and expose only a
-- tenant-checked, entity-allowlisted executor to the migration service.
